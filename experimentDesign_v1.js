...
const FIXED_AI_SUGGESTION_INDICES = [
    { index: 8, type: "false_neg" },
    { index: 12, type: "false_neg" },
    { index: 18, type: "false_pos" },
    { index: 20, type: "false_neg" },
    { index: 25, type: "false_pos" },
    { index: 36, type: "false_pos" },
    { index: 39, type: "false_pos" },
    { index: 40, type: "false_neg" },
    { index: 45, type: "false_neg" },
    { index: 48, type: "false_pos" }
];
//--------------------------------
...
function makeExpStruct() {
    ...
    //===========================================
    // Create AI experimental trials
    //===========================================
	let experimentTrialsAI = [];
    for (let trialType in CONFIG.experimentTrialsConditionsAI) {
        let readableTrialType;
        if (trialType === 'N_TARGET_PRESENT') readableTrialType = 'target_present';
        else if (trialType === 'N_TARGET_ABSENT') readableTrialType = 'target_absent';
        const totalTrialsForType = CONFIG.experimentTrialsConditionsAI[trialType];
        const trialsPerSetSize = totalTrialsForType / CONFIG.stimuli.SET_SIZE.length; 	// trials of different set sizes are evenly distributed across trial types
        for (const setSize of CONFIG.stimuli.SET_SIZE) {
            experimentTrialsAI.push(...createTrials(trialsPerSetSize, readableTrialType, trialID, setSize));
        }
    }
    //ADD AI Suggestion

    experimentTrialsAI = shuffle(experimentTrialsAI);
    experimentTrialsAI = applyFixedAISuggestions(experimentTrialsAI);
    //experimentTrialsAI = generateAISuggestionsTrial(experimentTrialsAI)

	// Create balanced blocks and push to expStruct
    // const propertiesToGroupBy = ['trialType', 'setSize'];
    // const experimentBlocksNoAI = distributeTrialsEqually(experimentTrialsNoAI, CONFIG.experimentDesign.N_NO_CAD_BLOCKS, propertiesToGroupBy);
    // const experimentBlocksNoAIBlock2 = distributeTrialsEqually(experimentTrialsNoAIBlock2, CONFIG.experimentDesign.N_NO_CAD_BLOCKS, propertiesToGroupBy);
    // const experimentBlocksAI = distributeTrialsEqually(experimentTrialsAI, CONFIG.experimentDesign.N_CAD_BLOCKS, propertiesToGroupBy);
    ...
    //const practiceBlocksAI = distributeTrialsEqually(practiceTrialsAI, CONFIG.experimentDesign.N_PRACTICE_BLOCKS, propertiesToGroupBy);
    ...
    //         return hardCodedIndices(index)
    //     } else {
    //         return grabRandomGeneratedItem() // the random 40
    //     }
    // })
